...
class World(object):
    def __init__(self, size, custom, start_position, start_direction_index, food_position):
        ...
        for i in range(self.size[0]):
            ...
            self.world[self.size[0] - 1][i] = self.WALL

        for i in range(1, self.size[1] - 1):
            self.world[i][0] = self.WALL
            self.world[i][self.size[1] - 1] = self.WALL
        ...
        for i in range(1, size[0] - 1):
            for j in range(1, size[1] - 1):
                self.available_food_positions.append((i, j))
        # Init snake
    ...
    def init_snake(self):
        ...
        if not self.custom:
            ...
            x = self.size[0] - SNAKE_SIZE
            y = self.size[1] - SNAKE_SIZE

            # Choose a random direction index
            start_direction_index = random.randint(0, 3)

            start_position = (
                random.randint(SNAKE_SIZE + 1, x), random.randint(SNAKE_SIZE + 1, y))
    ...
    def init_food(self):
        ...
        for i in range(len(self.available_food_positions)):
            flag = True
            for j in range(len(snake.blocks)):
                if self.available_food_positions[i] == snake.blocks[j]:
                    flag = False
                    break
            if flag:
                self.current_available_food_positions.add(self.available_food_positions[i])

        if not self.custom:
            # Choose a random position from available now
            secure_random = random.SystemRandom()
            chosen_position = secure_random.choice(list(self.current_available_food_positions))
        else:
    ...
    def move_snake(self, action):
        ...
        if self.snake.alive:
            ...
            if new_snake_head[0] == 0 or new_snake_head[0] == self.size[0] - 1 or new_snake_head[1] == 0 or \
                    new_snake_head[1] == self.size[1] - 1:
